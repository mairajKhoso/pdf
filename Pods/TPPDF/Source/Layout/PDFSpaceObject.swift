...
class PDFSpaceObject: PDFObject {
     ...
     */
    init(space: CGFloat, foreColor : UIColor) {
        self.space = space
    ...
    override func draw(generator: PDFGenerator, container: PDFContainer) throws {
        if generator.debug {
            PDFGraphics.drawRect(rect: self.frame, outline: PDFLineStyle(type: .dashed, color: .red, width: 1.0),
                                 pattern: PDFGraphics.FillPattern.dotted(foreColor: self.foreColor, backColor: .white))
        }
    }

    override var copy: PDFObject {
        return PDFSpaceObject(space: self.space, foreColor: self.foreColor)
    }
}
